...
class RemoveCameraPermissions : Patch<PatcherContext>() {

    override fun execute(context: PatcherContext) {
        // Fetch the manifest XML document context explicitly from the PatcherContext interface
        val manifestDoc: Document = context.manifest
        val root = manifestDoc.documentElement

        // 1. Remove the camera permission node safely by iterating downwards
        ...
        // 2. Remove the camera hardware requirement node safely by iterating downwards
